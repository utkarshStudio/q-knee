...
async function executeFallbackQuery(text: string, params: any[] = []): Promise<QueryResult<any>> {
  ...
  if (lowerSql.includes('select count(*) from studies')) {
    if (lowerSql.includes('user_id =') && params.length > 0) {
      ...
      const userStudies = memoryStore.studies.filter((s) => String(s.user_id) === userId);
      return { rows: [{ count: String(userStudies.length) }], command: 'SELECT', rowCount: 1, oid: 0, fields: [] };
    }
    return { rows: [{ count: String(memoryStore.studies.length) }], command: 'SELECT', rowCount: 1, oid: 0, fields: [] };
  }
  ...
  // 6. STUDIES: Select study by ID and user_id / ID
  if (lowerSql.includes('from studies') && lowerSql.includes('where')) {
    if (lowerSql.includes('id = $1') && lowerSql.includes('user_id = $2') && params.length >= 2) {
      const id = String(params[0]);
      const userId = String(params[1]);
      const study = memoryStore.studies.find((s) => String(s.id) === id && String(s.user_id) === userId);
      return {
        rows: study ? [study] : [],
        command: 'SELECT',
        rowCount: study ? 1 : 0,
    ...
    if (lowerSql.includes('where id =') || lowerSql.includes('where s.id =')) {
      const id = String(params[0]);
      const study = memoryStore.studies.find((s) => String(s.id) === id);
      return {
        rows: study ? [study] : [],
        command: 'SELECT',
        rowCount: study ? 1 : 0,
    ...
    if (lowerSql.includes('user_id =') || lowerSql.includes('s.user_id =')) {
      const userId = String(params[0]);
      let userStudies = memoryStore.studies.filter((s) => String(s.user_id) === userId);
      return {
        rows: userStudies,
        command: 'SELECT',
        rowCount: userStudies.length,
        oid: 0,
        fields: []
      };
    }
  ...
  if (lowerSql.startsWith('insert into studies')) {
    const newStudy: Study = {
      id: `study_${uuidv4().slice(0, 8)}`,
      user_id: params[0] || defaultUserId,
      study_instance_uid: params[1] || `uid_${Date.now()}`,
      original_filename: params[2] || 'study.npy',
      storage_reference: typeof params[3] === 'string' ? params[3] : JSON.stringify(params[3] || []),
      status: params[4] || 'processing',
      mode: params[5] || 'DEMO',
  ...
  if (lowerSql.startsWith('update studies')) {
    if (lowerSql.includes('set status = $1, metadata = $2')) {
      ...
      const study = memoryStore.studies.find((s) => String(s.id) === studyId);
      if (study) {
        study.status = status;
  ...
  if (lowerSql.startsWith('insert into predictions')) {
    const newPred: Prediction = {
      ...
      raw_output: typeof params[8] === 'string' ? JSON.parse(params[8] || '{}') : params[8],
      created_at: new Date().toISOString(),
    };
  ...
  if (lowerSql.startsWith('insert into explanations')) {
    const newExpl: Explanation = {
      ...
      gradcam_reference: typeof params[1] === 'string' ? JSON.parse(params[1] || '{}') : params[1],
      attribution_method: params[2] || 'sensitivity',
      attribution_data: typeof params[3] === 'string' ? JSON.parse(params[3] || '[]') : params[3],
      created_at: new Date().toISOString(),
    };
