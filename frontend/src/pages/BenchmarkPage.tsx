...
export default function BenchmarkPage() {
  ...
  const mc = (v: number | null | undefined) =>
    v === null || v === undefined ? (
      ...
      <span className="font-mono font-medium text-text-primary">{formatMetric(v, 3)}</span>
    );
      ...
      <div className="p-4 md:p-6 lg:p-8 space-y-6 max-w-[1400px] mx-auto">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4">
            <div className="flex bg-bg-panel rounded-lg p-1 border border-border-subtle text-xs font-medium w-full sm:w-auto">
              <button
                onClick={() => setActiveTab("metrics")}
                className={`flex-1 sm:flex-none px-4 py-2 rounded-md transition-all ${
                  activeTab === "metrics" ? "bg-clinical-600 text-white shadow-sm font-semibold" : "text-text-muted hover:text-white"
                }`}
              >
                Metrics Table
              </button>
              <button
                ...
                className={`flex-1 sm:flex-none px-4 py-2 rounded-md transition-all ${
                  activeTab === "charts" ? "bg-clinical-600 text-white shadow-sm font-semibold" : "text-text-muted hover:text-white"
                }`}
              >
                Visual Charts
              </button>
        ...
        <div className="card p-5 bg-clinical-950/20 border-clinical-900/50 flex flex-col sm:flex-row sm:items-center gap-4">
          ...
          <div>
            <h3 className="text-sm font-bold text-text-primary mb-1">Evaluation Methodology</h3>
        ...
        {svmBenchmark && vqcBenchmark && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {/* SVM Card */}
            <div className="card p-6 border-border-strong bg-gradient-to-br from-bg-panel to-bg-base relative overflow-hidden">
              <div className="absolute top-0 right-0 p-6 opacity-10">
                <Layers className="w-24 h-24 text-text-muted" />
              </div>
              <div className="flex items-start justify-between relative z-10 mb-6 border-b border-border-subtle pb-4">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-widest text-text-muted mb-1 block">Classical Baseline</span>
                  <h3 className="text-lg font-bold text-text-primary flex items-center gap-2">
                    <Activity className="w-5 h-5 text-text-muted" /> Support Vector Machine
                  </h3>
                </div>
              ...
              <div className="grid grid-cols-3 gap-4 relative z-10">
                <div className="bg-bg-base/50 p-4 rounded-xl border border-border-subtle text-center">
                  <div className="text-[10px] text-text-muted uppercase tracking-widest mb-1.5">Accuracy</div>
                  <div className="text-xl font-mono font-bold text-text-primary">{formatPercent(svmBenchmark.accuracy)}</div>
                </div>
                <div className="bg-bg-base/50 p-4 rounded-xl border border-border-subtle text-center">
                  <div className="text-[10px] text-text-muted uppercase tracking-widest mb-1.5">F1-Score</div>
                  <div className="text-xl font-mono font-bold text-text-primary">{formatMetric(svmBenchmark.f1, 3)}</div>
                </div>
                <div className="bg-bg-base/50 p-4 rounded-xl border border-border-subtle text-center">
                  <div className="text-[10px] text-text-muted uppercase tracking-widest mb-1.5">ROC-AUC</div>
                  <div className="text-xl font-mono font-bold text-text-primary">{formatMetric(svmBenchmark.roc_auc, 3)}</div>
                </div>
              </div>
            </div>

            {/* VQC Card */}
            <div className="card p-6 border-quantum-900/50 bg-gradient-to-br from-quantum-950/20 to-bg-base relative overflow-hidden">
              <div className="absolute top-0 right-0 p-6 opacity-10">
        ...
        {loading ? (
          ...
          <div className="card p-12 text-center bg-bg-panel/50 border-border-subtle">
            <BarChart3 className="w-12 h-12 text-text-disabled mx-auto mb-4" />
            <p className="text-text-primary font-bold text-lg mb-2">No Evaluations Recorded</p>
            <p className="text-text-muted text-sm max-w-md mx-auto">
              Click "Run Benchmark" above to evaluate Classical SVM vs Hybrid VQC on the held-out test split.
            </p>
          </div>
        ) : activeTab === "metrics" ? (
          <div className="card overflow-hidden border-border-subtle bg-bg-panel/50">
            <div className="p-5 border-b border-border-subtle flex items-center justify-between">
              <h2 className="text-sm font-bold text-text-primary uppercase tracking-wider">Evaluation History Log</h2>
              <span className="text-xs font-mono bg-bg-base px-2.5 py-1 rounded border border-border-strong text-text-muted">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-bg-base border-b border-border-subtle">
                  <tr>
                    {[
                      ...
                      <th
                        key={h}
                        className="text-left px-5 py-4 text-[10px] font-bold text-text-muted uppercase tracking-widest whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-graphite-800/50">
                  {benchmarks.map((b) => (
                    <tr key={b.id} className="hover:bg-bg-card/30 transition-colors group">
                      <td className="px-5 py-4 font-medium text-text-primary whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <span
                            className={`w-2 h-2 rounded-full ${
                              b.model_name === "ClassicalSVM" ? "bg-border-strong shadow-[0_0_8px_rgba(156,163,175,0.8)]" : "bg-quantum-500 shadow-[0_0_8px_rgba(168,85,247,0.8)]"
                            }`}
                          ></span>
                          {b.model_name}
                        </div>
                      ...
                      <td className="px-5 py-4 text-text-muted font-mono text-xs">{b.sample_count ?? "-"}</td>
                      <td className="px-5 py-4 text-text-muted text-xs font-mono whitespace-nowrap">{formatDate(b.created_at)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="card p-6 bg-bg-panel/50 border-border-subtle">
              <h3 className="text-sm font-bold text-text-primary uppercase tracking-wider mb-4 border-b border-border-subtle pb-3">
                Receiver Operating Characteristic (ROC)
              </h3>
              <div className="aspect-[5/3.5] w-full flex items-center justify-center bg-black rounded-xl overflow-hidden border border-border-subtle">
                <img
            ...
            <div className="card p-6 bg-bg-panel/50 border-border-subtle">
              ...
              <div className="aspect-[5/2.5] w-full flex items-center justify-center bg-black rounded-xl overflow-hidden border border-border-subtle">
                <img
            ...
            <div className="card p-6 bg-bg-panel/50 border-border-subtle lg:col-span-2">
              <h3 className="text-sm font-bold text-text-primary uppercase tracking-wider mb-4 border-b border-border-subtle pb-3">
                Metric-by-Metric Comparison
              </h3>
              <div className="aspect-[5/2.4] w-full flex items-center justify-center bg-black rounded-xl overflow-hidden border border-border-subtle">
                <img
